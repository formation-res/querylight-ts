...
describe("advanced query features", () => {
  it("should validate dis max tie breaker values", () => {
    expect(() => new DisMaxQuery([], -0.1)).toThrow();
    expect(() => new DisMaxQuery([], 1.1)).toThrow();
  });

  it("should validate boosting negative boost values", () => {
    expect(() => new BoostingQuery(new MatchAll(), new MatchAll(), 0)).toThrow();
    expect(() => new BoostingQuery(new MatchAll(), new MatchAll(), 1.1)).toThrow();
  });

  it("should validate distance feature parameters", () => {
    expect(() => new DistanceFeatureQuery("publishedAt", "not-a-date", 10)).toThrow();
    expect(() => new DistanceFeatureQuery("publishedAt", "2025-01-01T00:00:00.000Z", 0)).toThrow();
  });
  ...
  it("should support question mark wildcards", () => {
    ...
    expect(index.searchRequest({ query: new WildcardQuery("title", "c?t") }).map(([id]) => id).sort()).toEqual(["1", "2"]);
  });
  ...
  it("should support regex objects without global-state issues", () => {
    ...
    const query = new RegexpQuery("title", /^query/g);
  ...
  it("should support multiple numeric values per document for rank and distance features", () => {
    ...
    expect(index.searchRequest({ query: new RankFeatureQuery("feature") }).map(([id]) => id)).toEqual(["3", "1", "2"]);
    expect(index.searchRequest({ query: new DistanceFeatureQuery("feature", 7, 5) }).map(([id]) => id)).toEqual(["1", "2", "3"]);
  });
  ...
  it("should support numeric and date range queries on dedicated indexes", () => {
    ...
    expect(index.searchRequest({ query: new RangeQuery("price", { gte: "10", lt: "20" }) }).map(([id]) => id)).toEqual(["2"]);
    expect(index.searchRequest({
      query: new RangeQuery("publishedAt", { gte: "2025-01-05T00:00:00.000Z", lt: "2025-01-20T00:00:00.000Z" })
    }).map(([id]) => id)).toEqual(["2"]);
  });
  ...
  it("should support alternate rank feature modes", () => {
    ...
    expect(index.searchRequest({ query: new RankFeatureQuery("popularity", { type: "log", scalingFactor: 1 }) }).map(([id]) => id)).toEqual(["3", "2", "1"]);
    expect(index.searchRequest({ query: new RankFeatureQuery("popularity", { type: "sigmoid", pivot: 10, exponent: 2 }) }).map(([id]) => id)).toEqual(["3", "2", "1"]);
    expect(index.searchRequest({ query: new RankFeatureQuery("popularity", { type: "linear", factor: 0.5 }) }).map(([id]) => id)).toEqual(["3", "2", "1"]);
  });
  ...
  it("should expose index-backed numeric helpers to scripts", () => {
    ...
    const filtered = index.searchRequest({
      query: new ScriptQuery(({ numericValues }) => numericValues("popularity").some((value) => value >= 10))
    });
    ...
    const rescored = index.searchRequest({
      query: new ScriptScoreQuery(
        new TermQuery("title", "alpha"),
        ({ score, numericValues }) => score + numericValues("popularity").reduce((sum, value) => sum + value, 0)
      )
    });
