...
describe("advanced query features", () => {
  ...
  it("should support question mark wildcards", async () => {
    ...
    expect((await index.searchRequest({ query: new WildcardQuery({ field: "title", pattern: "c?t" }) })).map(([id]) => id).sort()).toEqual(["1", "2"]);
  });

  it("should support regex objects without global-state issues", async () => {
    const index = new DocumentIndex({ title: new TextFieldIndex() });
    ...
    expect((await index.searchRequest({ query })).map(([id]) => id).sort()).toEqual(["1", "2"]);
    expect((await index.searchRequest({ query })).map(([id]) => id).sort()).toEqual(["1", "2"]);
  });

  it("should return positive hits only for boosting queries", async () => {
    ...
    expect((await index.searchRequest({ query })).map(([id]) => id)).toEqual(["1"]);
  });

  it("should support multiple numeric values per document for rank and distance features", async () => {
    ...
    expect((await index.searchRequest({ query: new RankFeatureQuery({ field: "feature" }) })).map(([id]) => id)).toEqual(["3", "1", "2"]);
    expect((await index.searchRequest({ query: new DistanceFeatureQuery({ field: "feature", origin: 7, pivot: 5 }) })).map(([id]) => id)).toEqual(["1", "2", "3"]);
  });

  it("should support numeric and date range queries on dedicated indexes", async () => {
    ...
    expect((await index.searchRequest({ query: new RangeQuery({ field: "price", range: { gte: "10", lt: "20" } }) })).map(([id]) => id)).toEqual(["2"]);
  ...
  it("should support alternate rank feature modes", async () => {
    ...
    expect((await index.searchRequest({ query: new RankFeatureQuery({ field: "popularity", options: { type: "log", scalingFactor: 1 } }) })).map(([id]) => id)).toEqual(["3", "2", "1"]);
    expect((await index.searchRequest({ query: new RankFeatureQuery({ field: "popularity", options: { type: "sigmoid", pivot: 10, exponent: 2 } }) })).map(([id]) => id)).toEqual(["3", "2", "1"]);
    expect((await index.searchRequest({ query: new RankFeatureQuery({ field: "popularity", options: { type: "linear", factor: 0.5 } }) })).map(([id]) => id)).toEqual(["3", "2", "1"]);
  });

  it("should expose index-backed numeric helpers to scripts", async () => {
    ...
    const filtered = await index.searchRequest({
    ...
    const rescored = await index.searchRequest({
