...
describe("geo index", () => {
  it("test geo queries", () => {
    ...
    expect(index.searchRequest({ query: new GeoPointQuery("loc", 52.5, 13.4) }).map(([id]) => id)).toContain("berlin");
    expect(index.searchRequest({ query: new GeoPolygonQuery("loc", rectangleToPolygon(12.0, 52.0, 14.0, 53.0)) }).map(([id]) => id)).toEqual(["berlin"]);
  });
});
