...
describe("geo index", () => {
  it("test geo queries", async () => {
    ...
    expect((await index.searchRequest({ query: new GeoPointQuery({ field: "loc", latitude: 52.5, longitude: 13.4 }) })).map(([id]) => id)).toContain("berlin");
    expect((await index.searchRequest({ query: new GeoPolygonQuery({ field: "loc", polygon: rectangleToPolygon(12.0, 52.0, 14.0, 53.0) }) })).map(([id]) => id)).toEqual(["berlin"]);
  });
});
