...
- Use [Approximate Nearest Neighbor Vector Search](./approximate-nearest-neighbor-vector-search.md) for direct vector retrieval.
- Use [BoolQuery for Must, Should, Filter, MustNot, and MinimumShouldMatch](./../lexical-querying/bool-query.md) to build the cheap prefilter stage.
- Use [Geo Indexing with Points and Polygons](./geo-indexing-with-points-and-polygons.md) when a map or region constraint should narrow the candidate set before vector scoring.
- Use [Reciprocal Rank Fusion](./../ranking/reciprocal-rank-fusion.md) when you want to combine separate ranked lists instead of rescoring one candidate window.
- Use [Relevance Tuning with BM25, TF-IDF, and RRF](./../ranking/relevance-tuning.md) when ranking quality is the broader problem.
